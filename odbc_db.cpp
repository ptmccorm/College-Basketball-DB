...
string builder = ""; 
   try 
   {
	  ...
	  //builder.append("Query: <br>" + q + "<br><br>Result: <br>");
...
void odbc_db::insert(string table, string values) 
{
   ...
   if(table == "TEAM")
   {
		header = "UNIVERSITY_NAME,NICKNAME,RANK";
		query = "INSERT into " + table + "(" + header + ")" + " values (" + values + ")";
   }
   else if(table == "GAME")
   {
		header = "HOME_TEAM,OPPONENT,LOCATION,PLAY_DATE";
		query = "INSERT into " + table + "(" + header + ")" + " values (" + values + ")";
   }
   else
